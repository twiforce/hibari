...
const noAuthPath = /^\/(css.*|js.*|img.*|.*\.txt|sioconfig)$/;
...
function handleChannel(req, res) {
    ...
    req.locals.sioSource = sio;
    req.locals.channelName = req.params.channel;

    sendJade(res, "channel", req.locals);
}
...
function handleUserAgreement(req, res) {
    ...
    req.locals.domain = Config.get("http.domain");

    sendJade(res, "tos", req.locals);
}
...
function handleContactPage(req, res) {
    ...
    req.locals.contacts = contacts;

    sendJade(res, "contact", req.locals);
}
...
function addAuthData(req, res, next) {
    req.locals = req.locals || {};
    req.locals.loggedIn = false;
    req.locals.loginName = "";

    if (req.path.match(noAuthPath)) {
        return next();
    }

    var loginCookie = null;
    if (req.signedCookies.auth) {
        loginCookie = req.signedCookies.auth;
    } else if (req.cookies.auth) {
        loginCookie = req.cookies.auth;
    }
    ...
    db.users.verifyAuth(loginCookie, function (err, user) {
        if (err) {
...
module.exports = {
    ...
    init: function (app) {
        ...
        app.use(express.cookieParser(Config.get("http.cookie-secret")));
        app.use(addAuthData);
